...
import { makeBoat, drawBoats, drawWalkers, makeWalker, type Boat, type Walker } from "./boats";
import { GROUND } from "./layout";
import { css, hex, mix, palette, smooth, type Palette, type RGB } from "./sky";
import { bridgeSpan, drawRiver, drawSprite, newRiver, newSprite, renderBank, renderPark } from "./waterfront";
...
const MARGIN_Y = 8;
// far, mid, near towers; then the bank (+bridge) and the park in front
const PARALLAX_X = [5, 10, 18];
const PARALLAX_Y = [1, 2, 3];
const BANK_X = 22;
const BOAT_X = 27;
const PARK_X = 34;
const PARK_Y = 6;
...
function startScene(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D): () => void {
  ...
  const bank = newSprite();
  const park = newSprite();
  const river = newRiver();
  ...
  const next = { palette: t0 + 30, toggle: t0 + between(2, 4), flock: t0 + between(2, 6), plane: t0 + between(4, 12), shoot: t0 + between(6, 16), boat: t0 + between(1, 4), walker: t0 + between(1, 3) };

  function renderLayers(which: number[] = [0, 1, 2]) {
    const lw = W + MARGIN_X * 2;
    const lh = GROUND * H + MARGIN_Y * 2;
    const sx = Math.max(lw, 900) / CITY_W;
    const ox = (lw - CITY_W * sx) / 2;
    for (const i of which) {
      const c = layerCtx[i];
      c.setTransform(dpr, 0, 0, dpr, 0, 0);
      winKeys[i] = renderLayer(c, city[i], i, { width: lw, H, groundY: lh - MARGIN_Y, sx, ox }, pal, toggled);
    }
  }
  ...
  function refreshPalette(force = false) {
    ...
    if (force || Math.abs(hour - renderedHour) > 0.08 || Math.abs(pal.darkness - renderedDark) > 0.02) {
      ...
      renderLayers();
      renderClouds();
      renderBank(bank, W, H, dpr, pal);
      renderPark(park, W, H, dpr, pal);
    }
  }
  ...
  function resize() {
    ...
    for (const c of layers) {
      ...
      c.height = Math.round((GROUND * H + MARGIN_Y * 2) * dpr);
    }
    refreshPalette(true);
  }
  ...
  function tick(t: number) {
    ...
    if (t >= next.boat) {
      next.boat = t + between(14, 30);
      if (boats.length < 2) boats.push(makeBoat(t, W, H, d));
    }
    if (t >= next.walker) {
      next.walker = t + between(5, 12);
      if (walkers.length < 4) walkers.push(makeWalker(t));
    }
    if (t >= next.shoot) {
  ...
  function drawOrb() {
    ...
    const y = H * (GROUND + 0.02) - arc * H * 0.6;
  ...
  function draw(t: number) {
    ...
    const hz = ctx.createLinearGradient(0, H * 0.4, 0, H * GROUND);
    hz.addColorStop(0, css(pal.haze, 0));
    hz.addColorStop(1, css(pal.haze, 0.45));
    ctx.fillStyle = hz;
    ctx.fillRect(0, H * 0.4, W, H * (GROUND - 0.4));
    ...
    const bankX = -pointer.x * BANK_X;
    drawSprite(ctx, bank, bankX);
    if (motion) drawWalkers(ctx, walkers, t, bridgeSpan(W, H), bankX, pal);
    // The river mirrors whatever is on the canvas above it, so it must come after all of that.
    drawRiver(ctx, canvas, river, W, H, dpr, pal, t, motion);
    if (motion) drawBoats(ctx, boats, t, W, dpr, -pointer.x * BOAT_X, pal);
    drawSprite(ctx, park, -pointer.x * PARK_X, -pointer.y * PARK_Y);
  }
